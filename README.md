...
- Structured child results (JSON-schema validated with one repair round), fan-out/fan-in spawn graphs, and a consent-based agent link registry — links are created in the Desktop's per-tab **Links** dialog (🔗 rail entry: pick the target agent, name the link, confirm; revoke from the same list), and `agent.route` delivers to the consented link by name
...
| Sub-agent watchdog (`TickInterval`, `IdleThreshold`, `MaxWrapUpAttempts`) | `appsettings.json` (`SubAgent:Watchdog` section), overridden by `SubAgent__Watchdog__*` environment variables | Tunes the watchdog that restarts or fails hung children — including the one running inside `eThangAgent.ChildHost` in remote mode. Durations use constant format (`00:00:02`); every key is optional and absent keys keep the defaults (15-minute idle threshold, 60-second tick, 1 wrap-up attempt). Invalid text aborts startup — never clamped, never coerced (bare integers are rejected: without units they would bind as days). |
