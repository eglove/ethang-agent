...
public sealed class FileEditIntegrationTests : IDisposable
{
  ...
  public async Task SingleOccurrence_Replaced_ReportsNewLineCount()
  {
    ...
    Result<ReplaceOutcome> r = await _access.ReplaceInFileAsync(p, "two", "TWO", occurrences: 1, ct: TestContext.Current.CancellationToken);
    Assert.True(r.IsSuccess);
    Assert.Equal(1, r.Value.Replaced);
    Assert.Equal(3, r.Value.NewLineCount);
    Assert.Equal("one\nTWO\nthree", await File.ReadAllTextAsync(p, TestContext.Current.CancellationToken));
  }
  ...
  public async Task ReplaceAll_NullOccurrences_ReplacesEveryMatch()
  {
    ...
    Result<ReplaceOutcome> r = await _access.ReplaceInFileAsync(p, "x", "y", occurrences: null, ct: TestContext.Current.CancellationToken);
    Assert.True(r.IsSuccess);
    Assert.Equal(3, r.Value.Replaced);
    Assert.Equal("y-y-y", await File.ReadAllTextAsync(p, TestContext.Current.CancellationToken));
  }
  ...
  public async Task OccurrenceMismatch_RequestedMoreThanExists_Fails()
  {
    ...
    Result<ReplaceOutcome> r = await _access.ReplaceInFileAsync(p, "one", "1", occurrences: 2, ct: TestContext.Current.CancellationToken);
  ...
  public async Task AnchorMissing_Fails_AnchorNotFound()
  {
    ...
    Result<ReplaceOutcome> r = await _access.ReplaceInFileAsync(p, "absent", "z", occurrences: 1, ct: TestContext.Current.CancellationToken);
    Assert.False(r.IsSuccess);
  ...
  public async Task BinaryFile_Fails_BinaryFile()
  {
    ...
    Result<ReplaceOutcome> r = await _access.ReplaceInFileAsync(p, "a", "b", occurrences: 1, ct: TestContext.Current.CancellationToken);
    Assert.False(r.IsSuccess);
  ...
  public async Task MissingFile_Fails_FileNotFound()
  {
    Result<ReplaceOutcome> r = await _access.ReplaceInFileAsync(
        Path.Combine(_root, "ghost.txt"), "a", "b", occurrences: 1, ct: TestContext.Current.CancellationToken);
    Assert.False(r.IsSuccess);
