...
public class DesktopAgentCapabilityE2ETests
{
  ...
  public async Task StateDisciplineLoop_Certifies()
  {
    ...
    _ = host.Mock.Returns(RawCompletion("certified"));
  ...
  public async Task StateDisciplineLoop_Violated_OnFailingEvidence()
  {
    ...
    string program = """
            ...
            """;
    _ = host.Mock.Returns(E2E.ExecToolCall("call_1", E2E.ExecProgram(program)));
    _ = host.Mock.Returns(RawCompletion("violated"));
  ...
  public async Task TodoToolWritesFlow_ButModelStateWritesOnTodoNs_AreRejected()
  {
    ...
    _ = host.Mock.Returns(E2E.ExecToolCall("call_1", E2E.ExecProgram("Tools.Invoke(\"todo\", new { timeoutSeconds = 120, action = \"Add\", description = \"ship it\" })")));
    _ = host.Mock.Returns(E2E.ExecToolCall("call_2", E2E.ExecProgram("Tools.Invoke(\"state.set\", new { timeoutSeconds = 60, key = \"todo/list\", value = \"hijack\" })")));
    _ = host.Mock.Returns(E2E.ExecToolCall("call_3", E2E.ExecProgram("Tools.Invoke(\"state.delete\", new { timeoutSeconds = 60, key = \"todo/list\" })")));
    _ = host.Mock.Returns(E2E.ExecToolCall("call_4", E2E.ExecProgram("Tools.Invoke(\"todo\", new { timeoutSeconds = 120, action = \"List\" })")));
  ...
  public async Task MemoryRecall_AgainstMockServer()
  {
    ...
    _ = host.Mock.Returns(RawCompletion("The xylophone harvest begins at dawn."));
    await host.Vm.RunTurnAsync("tell me about the xylophone harvest");

    // Turn 2: one exec tool call listing what conversations exist.
    _ = host.Mock.Returns(E2E.ExecToolCall("call_1", E2E.ExecProgram("return Tools.Invoke(\"memory.sessions\", new { timeoutSeconds = 60, limit = 50 });")));
    // Turn 3: one exec tool call recalling the seeded phrase across all sessions.
    _ = host.Mock.Returns(E2E.ExecToolCall("call_2", E2E.ExecProgram("return Tools.Invoke(\"memory.recall\", new { timeoutSeconds = 60, query = \"xylophone\", scope = \"global\" });")));
    // Turn 4: final text closes the exchange.
    _ = host.Mock.Returns(RawCompletion("recalled."));
    ...
    string recallOutput = E2E.FindToolMessageContaining(host.Mock.RequestBodies, "xylophone harvest");
