...
public class StateServiceAppendTests
{
    ...
    public async Task Append_ToExistingKey_AddsLine_AndBumpsVersion()
    {
        ...
        var r = await Service().AppendAsync("sdd.x/ledger", "line two", expectedVersion: 3);
    ...
    {
        var r = await Service().AppendAsync("sdd.y/ledger", "first line", null);
        Assert.True(r.IsSuccess);
    ...
    public async Task Append_VersionConflict_FailsClosed()
    {
        ...
        var r = await Service().AppendAsync("sdd.x/ledger", "line", expectedVersion: 9);
    ...
    {
        var r = await Service().AppendAsync("sdd.x/ledger", text, null);
        Assert.False(r.IsSuccess);
    ...
    public async Task DeletePrefix_RemovesOnlyMatchingNamespace()
    {
        ...
        var r = await Service().DeletePrefixAsync("sdd.alpha");
        Assert.True(r.IsSuccess);
    ...
    {
        public Dictionary<string, StateKeyValue> Keys { get; } = new();
        ...
        public Task<StateKeyValue?> GetKeyAsync(string workspaceId, string ns, string name, CancellationToken ct = default)
            => Task.FromResult(Keys.TryGetValue($"{workspaceId}|{ns}/{name}", out var row) ? row : null);
        ...
        public Task<IReadOnlyList<StateKeyValue>> ListKeysAsync(string workspaceId, string? ns, CancellationToken ct = default)
            => Task.FromResult<IReadOnlyList<StateKeyValue>>(
                Keys.Where(k => k.Key.StartsWith(workspaceId + "|"))
        ...
        {
            if (Conflict) return Task.FromResult<StateKeyValue?>(null);
            var key = $"{workspaceId}|{ns}/{name}";
            var version = Keys.TryGetValue(key, out var row) ? row.Version + 1 : 1;
            var saved = new StateKeyValue(ns, name, value, version);
            Keys[key] = saved;
        ...
        {
            var removed = 0;
            foreach (var key in Keys.Keys.Where(k => k.StartsWith(workspaceId + "|") &&
                                                    (k[(workspaceId.Length + 1)..].Split('/')[0] == nsPrefix ||
                                                     k[(workspaceId.Length + 1)..].Split('/')[0].StartsWith(nsPrefix + "."))).ToList())
            { if (Keys.Remove(key)) removed++; }
