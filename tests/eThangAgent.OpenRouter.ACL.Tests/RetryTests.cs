...
// Transient OpenRouter failures (429/408/5xx, transport errors, timeouts) are retried
...
public class RetryTests
{
    ...
    private sealed class Recorder
    {
        ...
        public HttpClient Client()
        {
            var handler = new FakeHttpMessageHandler(_ =>
            {
                ...
                return RespondAsync is not null ? RespondAsync(Calls) : Task.FromResult(Respond(Calls));
            });
            return new HttpClient(handler);
        }
    }

    private static OpenRouterModelProvider Provider(OpenRouterConfiguration config, Recorder rec) =>
        new(rec.Client(), config,
            delay: (t, _) => { rec.Delays.Add(t); return Task.CompletedTask; },
            jitter: () => 0.0);
    ...
    {
        var rec = new Recorder { Respond = call => call switch { 1 or 2 => Status(HttpStatusCode.InternalServerError), _ => JsonOk() } };
        var provider = Provider(Config(new RetryPolicy(4)), rec);

        var result = await provider.SendAsync(Model, new ModelRequest([UserMsg("hi")]));

        Assert.True(result.IsSuccess);
        Assert.Equal(3, rec.Calls);
    }

    [Fact]
    public async Task RateLimit429_IsRetried_ThenSucceeds()
    {
        var rec = new Recorder { Respond = call => call == 1 ? Status(HttpStatusCode.TooManyRequests) : JsonOk() };
        var provider = Provider(Config(), rec);
    ...
    {
        var rec = new Recorder { Respond = _ => Status(HttpStatusCode.BadRequest) };
        var provider = Provider(Config(), rec);
    ...
    {
        var rec = new Recorder { Respond = _ => Status(HttpStatusCode.Unauthorized) };
        var provider = Provider(Config(), rec);
    ...
    {
        var rec = new Recorder { Respond = _ => Status(HttpStatusCode.InternalServerError) };
        var provider = Provider(Config(new RetryPolicy(3)), rec);
    ...
    {
        var rec = new Recorder { RespondAsync = call => call == 1 ? throw new HttpRequestException("boom") : Task.FromResult(JsonOk()) };
        var provider = Provider(Config(), rec);
    ...
    {
        var rec = new Recorder { RespondAsync = call => call == 1 ? throw new TaskCanceledException() : Task.FromResult(JsonOk()) };
        var provider = Provider(Config(), rec);
    ...
    public async Task Backoff_Doubles_PerAttempt()
    {
        ...
        await provider.SendAsync(Model, new ModelRequest([UserMsg("hi")]));
    ...
    {
        var content = new StreamContent(new DyingSseStream(System.Text.Encoding.UTF8.GetBytes(
    ...
    private sealed class DyingSseStream(byte[] prefix) : Stream
    {
        ...
        private int ReadCore(Span<byte> target)
        {
            ...
            var n = Math.Min(target.Length, prefix.Length);
