...
{
    private static string Render(params IMarkdownBlock?[] blocks) =>
    ...
    [Fact]
    public void SpaceCount_EmitsCountPlusOneNewlines()
    {
    ...
    {
        var doc = MarkdownRenderer.Render(new MarkdownDocument(
            [new TextBlock("Body")],
    ...
    {
        var doc = MarkdownRenderer.Render(new MarkdownDocument([], new Dictionary<string, object> { ["k"] = "v" }));
        Assert.Equal("---\nk: v\n---\n", doc);
    }
    ...
    {
        var doc = MarkdownRenderer.Render(new MarkdownDocument([], new Dictionary<string, object> { ["k"] = value }));
        Assert.Equal("---\nk: " + expected + "\n---\n", doc);
    }
    ...
    {
        var doc = MarkdownRenderer.Render(new MarkdownDocument([], new Dictionary<string, object> { ["weight"] = 80.0 }));
        Assert.Equal("---\nweight: 80\n---\n", doc);
    }
}
