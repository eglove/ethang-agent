...
public class GitmojiCatalogTests
{
    ...
    {
        var result = GitmojiCatalog.Lookup(":tada:");
    ...
    {
        var result = GitmojiCatalog.Lookup(":definitely_not_a_gitmoji:");
        Assert.False(result.IsSuccess);
        Assert.Equal("UnknownEmojiKey", result.Error!.Code);
        // First three keys of the embedded table, in file order.
        Assert.Multiple(
            () => Assert.Contains(":tada:", result.Error!.Message),
            () => Assert.Contains(":art:", result.Error.Message),
            () => Assert.Contains(":sparkles:", result.Error.Message),
            () => Assert.Contains("66", result.Error.Message));
    }
