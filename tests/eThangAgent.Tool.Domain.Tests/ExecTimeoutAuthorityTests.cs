...
public class ExecTimeoutAuthorityTests
{
    ...
    {
        Assert.DoesNotContain("hard cap", ExecGuide.Text);
        Assert.Contains("timeoutSeconds is the only execution budget", ExecGuide.Text);
        Assert.Contains("Error [ToolTimeout]", ExecGuide.Text);
    }

    [Fact]
    public void ToolDescription_NamesPerCallBudgetAsSoleBudget_NoHardCap()
    {
        var description = new ExecTool(
            new StubExecEngine(), ExecOptions.Default,
            new StubOutputStore(), NullExecActivitySink.Instance).Definition.Description;

        Assert.DoesNotContain("hard cap", description);
        Assert.Contains("timeoutSeconds is the only execution budget", description);
    }

    [Fact]
    public void Guide_VersionBumped()
    {
