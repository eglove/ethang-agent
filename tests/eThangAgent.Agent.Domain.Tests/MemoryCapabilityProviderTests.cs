...
public class MemoryCapabilityProviderTests
{
    ...
    {
        public int Calls;
        public (string? Query, string QueryMode, string? Scope, string Branches, string? Role,
            int Page, int PageSize)? LastArgs;
        public CancellationToken LastCt;
        private readonly Result<RecallPage> _reply =
            reply ?? Result<RecallPage>.Success(new RecallPage([], 0, 1, 1));
        ...
        {
            Calls++;
            LastArgs = (query, queryMode, scope, branches, role, page, pageSize);
            LastCt = ct;
    ...
    {
        public int Calls;
        public (string? Scope, string Branches, int Limit)? LastArgs;
        public CancellationToken LastCt;
        private readonly Result<IReadOnlyList<SessionSummary>> _reply =
            reply ?? Result<IReadOnlyList<SessionSummary>>.Success([]);
    ...
    {
        var recall = new FakeRecallQuery(recallReply);
        var sessions = new FakeSessionsQuery(sessionsReply);
        return (new MemoryCapabilityProvider(recall, sessions), recall, sessions);
    }

    private static RecallPage PageOf(params RecallPage.Hit[] hits)
        => new(hits, TotalMatched: 12, Page: 3, Pages: 3);

    [Fact]
    public async Task Recall_WithHits_RendersHitLinesAndFooterArithmetic()
    {
        var first = new AgentId(Guid.Parse("11111111-1111-1111-1111-111111111111"));
        var second = new AgentId(Guid.Parse("22222222-2222-2222-2222-222222222222"));
        var (provider, recall, _) = MakeProvider(Result<RecallPage>.Success(PageOf(
            new RecallPage.Hit(first, 2, "user", "hello world", DateTimeOffset.UtcNow),
            new RecallPage.Hit(second, 7, "assistant", "second line", DateTimeOffset.UtcNow))));

        var result = await provider.InvokeAsync("recall", """{"pageSize":5,"page":3}""", AnyToken);
        ...
        Assert.Equal(
            ...
            result.Content);
        Assert.NotNull(recall.LastArgs);
        Assert.Equal((null, "literal", "global", "active", null, 3, 5), recall.LastArgs);
        Assert.Equal(AnyToken, recall.LastCt);
    }
    ...
    public async Task Recall_WithZeroHits_RendersFooterOnly()
    {
        ...
        var result = await provider.InvokeAsync("recall", "{}");
    ...
    public async Task Recall_Snippet_CollapsesNewlinesAndTruncatesAt120Chars()
    {
        ...
        var session = new AgentId(Guid.Parse("33333333-3333-3333-3333-333333333333"));
        var content = "alpha\rbravo\ncharlie " + new string('z', 118);
        var (provider, _, _) = MakeProvider(Result<RecallPage>.Success(PageOf(new RecallPage.Hit(session, 4, "tool", content, DateTimeOffset.UtcNow))));
    ...
    {
        var first = new AgentId(Guid.Parse("44444444-4444-4444-4444-444444444444"));
        var second = new AgentId(Guid.Parse("55555555-5555-5555-5555-555555555555"));
        var (provider, _, sessions) = MakeProvider(sessionsReply: Result<IReadOnlyList<SessionSummary>>.Success(
        [
            new SessionSummary(first, "fix login loop", 1, 4, "Completed", "hot"),
            new SessionSummary(second, "", 0, 0, "Running", "hot"),
        ]));

        var result = await provider.InvokeAsync("sessions", "{}", AnyToken);
        ...
        Assert.Equal(
            ...
            result.Content);
        Assert.Equal(("global", "active", 50), sessions.LastArgs);
        Assert.Equal(AnyToken, sessions.LastCt);
    }

    [Fact]
    public async Task Recall_ForwardsDocumentedDefaults_ForEmptyArguments()
    {
        var (provider, recall, _) = MakeProvider();

        _ = await provider.InvokeAsync("recall", "{}");

        Assert.Equal(1, recall.Calls);
        Assert.Equal((null, "literal", "global", "active", null, 1, 25), recall.LastArgs);
    }

    [Fact]
    public async Task Sessions_ForwardsDocumentedDefaults_ForEmptyArguments()
    {
        var (provider, _, sessions) = MakeProvider();
    ...
    public async Task Recall_UnknownArgument_IsRejectedWithoutCallingTheQuery()
    {
        ...
        var result = await provider.InvokeAsync("recall", """{"bogus":1}""");

        Assert.True(result.IsError);
        Assert.Equal("Error [InvalidArgument]: unknown argument 'bogus'.", result.Content);
        Assert.Equal(0, recall.Calls);
    }
    ...
    public async Task Sessions_UnknownArgument_IsRejectedWithoutCallingTheQuery()
    {
        ...
        var result = await provider.InvokeAsync("sessions", """{"since":"yesterday"}""");
    ...
    public async Task Recall_PageSizeAsJsonString_IsRejected()
    {
        ...
        var result = await provider.InvokeAsync("recall", """{"pageSize":"5"}""");
    ...
    public async Task Recall_QueryAsJsonNumber_IsRejected()
    {
        ...
        var result = await provider.InvokeAsync("recall", """{"query":7}""");
    ...
    public async Task Sessions_LimitAsJsonString_IsRejected()
    {
        ...
        var result = await provider.InvokeAsync("sessions", """{"limit":"50"}""");
    ...
    public async Task Recall_NonObjectArguments_AreRejected()
    {
        ...
        var result = await provider.InvokeAsync("recall", "[1]");
    ...
    {
        var (provider, _, _) = MakeProvider(recallReply: Result<RecallPage>.Failure(new Error(
            "InvalidScope",
            "Unknown scope 'session:nope'. Valid scopes: global | session:<agentId>.")));

        var result = await provider.InvokeAsync("recall", """{"scope":"session:nope"}""");
    ...
    {
        var (provider, _, _) = MakeProvider(sessionsReply: Result<IReadOnlyList<SessionSummary>>.Failure(
            new Error("StorageUnavailable", "database locked.")));

        var result = await provider.InvokeAsync("sessions", "{}");
    ...
    public async Task UnknownAction_YieldsStandardError()
    {
        ...
        var result = await provider.InvokeAsync("forget", "{}");
