...
public class CuratedMemoryTests
{
    ...
    [InlineData("a")]
    [InlineData("0")]
    public void ValidTag_SingleCharacter_IsAccepted(string tag)
    {
        Assert.True(CuratedMemorySpecifications.ValidTag(tag));
    }

    [Theory]
    [InlineData("-abc")]
    [InlineData("-")]
    public void ValidTag_LeadingDash_IsRejected(string tag)
    {
        Assert.False(CuratedMemorySpecifications.ValidTag(tag));
    }

    [Theory]
    [InlineData("_abc")]
    [InlineData("_")]
    public void ValidTag_LeadingUnderscore_IsRejected(string tag)
    {
        Assert.False(CuratedMemorySpecifications.ValidTag(tag));
    }
    ...
    [Fact]
    public void ValidTag_33Chars_IsRejected()
    {
        Assert.False(CuratedMemorySpecifications.ValidTag(new string('a', 33)));
    }
    ...
    [InlineData("a b")]
    [InlineData("café")]
    public void ValidTag_InvalidCharset_IsRejected(string tag)
    {
        Assert.False(CuratedMemorySpecifications.ValidTag(tag));
    }
    ...
    {
        var normalized = CuratedMemorySpecifications.NormalizeTags(
            ["beta", "alpha", "beta", "alpha"]);

        Assert.Equal(new[] { "beta", "alpha" }, normalized);
    }
    ...
    public void NormalizeTags_DedupIsExactOrdinal_NotSimilarityBased()
    {
        ...
        Assert.Equal(new[] { "tag-1", "tag_1", "tag-11" }, normalized);
    }
    ...
    public void NormalizeTags_PreservesFirstSeenOrder()
    {
        ...
        Assert.Equal(new[] { "c", "a", "b" }, normalized);
    }
    ...
    {
        var result = CuratedMemorySpecifications.ParseCategory(raw);
    ...
    {
        var result = CuratedMemorySpecifications.ParseScope(raw);
    ...
    [Fact]
    public void MaxContentChars_Is4000()
    {
        Assert.Equal(4000, CuratedMemorySpecifications.MaxContentChars);
    }
    ...
    {
        var original = new CuratedMemory(
            Guid.NewGuid(),
            "ws-1",
        ...
        var mutated = original with { Version = 4 };
