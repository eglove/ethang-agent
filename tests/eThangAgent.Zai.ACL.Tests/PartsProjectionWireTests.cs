...
// Spec 5.3 z.ai pins: z.ai's OpenAI-compatible chat API cannot accept images on tool
// results, so the provider projects them - tool content serializes flat text and ALL
// image parts of the turn's tool results merge into ONE synthetic user message placed
// after the LAST tool result of the assistant tool_calls block, each image preceded by
// "[computer screenshot for tool call <toolCallId>]". Exactly one synthetic message per
// image-carrying turn; none when the turn has no images.
