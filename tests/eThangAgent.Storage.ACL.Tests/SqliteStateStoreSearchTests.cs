...
public class SqliteStateStoreSearchTests : IDisposable
{
    ...
    {
        await _store.SetKeyCasAsync("ws1", "plans", "alpha", "rewrite the SDD ledger flow", null);
        var r = await _store.SearchKeysAsync("ws1", "ledger", 20);
        Assert.True(r.IsSuccess);
        var hit = Assert.Single(r.Value!);
    ...
    {
        await _store.SetKeyCasAsync("ws1", "specs", "2026-08-24-native-skills-db-planning", "body text here", null);
        var r = await _store.SearchKeysAsync("ws1", "native* AND skills*", 20);
        Assert.True(r.IsSuccess);
    ...
    {
        await _store.SetKeyCasAsync("ws1", "notes", "mine", "xylophone collection", null);
        await _store.SetKeyCasAsync("ws2", "notes", "theirs", "xylophone collection", null);
        var r = await _store.SearchKeysAsync("ws1", "xylophone", 20);
        Assert.True(r.IsSuccess);
        var hit = Assert.Single(r.Value!);
    ...
    {
        await _store.SetKeyCasAsync("ws1", "notes", "a", "content", null);
        var r = await _store.SearchKeysAsync("ws1", "AND (", 20);
    ...
    {
        for (var i = 0; i < 5; i++)
            await _store.SetKeyCasAsync("ws1", "notes", $"k{i}", "shared term everywhere", null);
        var r = await _store.SearchKeysAsync("ws1", "shared", 3);
        Assert.True(r.IsSuccess);
