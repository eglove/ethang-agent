...
public class SqliteAgentStoreConcurrencyTests : IDisposable
{
    ...
    public async Task ConcurrentSavesAndAppends_AllSucceedAndPersist()
    {
        ...
        var saves = await Task.WhenAll(records.Select(
            r => Task.Run(() => _store.SaveAsync(r))));

        var shared = AgentRecord.Spawned(
        ...
        var messages = Enumerable.Range(0, 10)
            ...
            .ToArray();
        var appends = await Task.WhenAll(messages.Select(
        ...
        var transcript = await _store.GetTranscriptAsync(shared.Id);
