...
public sealed class SessionsQueryHandler
{
    ...
    public SessionsQueryHandler(IAgentStore store)
        ...
        string? scope, string branches, int limit)
    {
        ...
        if (!parsedScope.IsSuccess)
            return Result<IReadOnlyList<SessionSummary>>.Failure(parsedScope.Error!);

        var branchKnown = string.Equals(branches, "active", StringComparison.Ordinal) ||
    ...
    private static Result<IReadOnlyList<SessionSummary>> InvalidArgument(string message)
        => Result<IReadOnlyList<SessionSummary>>.Failure(new Error("InvalidArgument", message));
}
