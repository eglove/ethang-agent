...
| Store or read specs, plans, ledgers, briefs, reports | `state` tools — `state.get` / `state.set` / `state.append` (CAS ledger lines) / `state.list` / `state.find` (full-text search) / `state.prune` (SDD cleanup); design specs and implementation plans are plan records via the 'plan' provider ('plan create'/'plan get') |
