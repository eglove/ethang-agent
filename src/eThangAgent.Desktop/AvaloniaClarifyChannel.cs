...
/// </summary>
internal sealed class AvaloniaClarifyChannel(Func<ClarifyQuestion, Task<ClarifyViewModel>>? present)
    : IClarifyChannel
{
  ...
  public async Task<Result<string>> AskAsync(ClarifyQuestion question, CancellationToken ct = default)
  {
    if (_present is null)
    {
      return Result.Failure<string>(new DomainError("PresenterUnavailable",
          "No clarify presenter is attached in this context. Ask your question directly in chat instead."));
    }

    ClarifyViewModel vm = await _present(question);
    ...
    using CancellationTokenRegistration reg = ct.Register(vm.Cancel);
