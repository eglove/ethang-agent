...
public static class ToolTimeout
{
    ...
    {
        if (!json.TryGetProperty(ParameterName, out var el))
            return Result<TimeSpan>.Failure(new Error("MissingParameter",
                "Missing required parameter '" + ParameterName +
                "'. Every tool call must state its execution budget in seconds."));

        if (el.ValueKind != JsonValueKind.Number || !el.TryGetInt32(out var seconds))
            return Result<TimeSpan>.Failure(new Error("InvalidParameterType",
                "'" + ParameterName + "' must be an integer number of seconds, but got " +
                el.ValueKind + "."));

        if (seconds < 1)
            return Result<TimeSpan>.Failure(new Error("InvalidParameterValue",
    ...
    public static ToolResult TimedOut(string toolName, TimeSpan budget) =>
        new("Error [ToolTimeout]: '" + toolName + "' exceeded its " +
            budget.TotalSeconds.ToString("0") + "s execution budget and was stopped. " +
