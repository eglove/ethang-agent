...
public sealed class WriteMarkdownTool(IPathResolver resolver, IFileWriteAccess files) : ITool, IWorkspaceScopedTool
{
  ...
  public ITool RootedAt(string workspaceRoot)
      => new WriteMarkdownTool(new WorkspacePathResolver(workspaceRoot), _files);
