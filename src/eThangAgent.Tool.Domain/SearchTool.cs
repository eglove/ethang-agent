...
public sealed class SearchTool : ITool
{
    ...
    public ToolDefinition Definition { get; } = new(
        "search_files",
        "Search workspace text files for a pattern. pattern, mode, and maxResults are mandatory; " +
    ...
    public async Task<ToolResult> ExecuteAsync(RawToolInput input, CancellationToken ct = default)
    {
        ...
        var found = await _search.SearchFilesAsync(
            scope.Value!, v.Pattern, v.Regex, v.Glob, v.MaxResults, v.ContextLines, ct);
        if (!found.IsSuccess)
        ...
        if (o.Matches.Count == 0)
        {
            return new ToolResult(
                $"[search '{v.Pattern}' under {scope.Value}: no matches ({o.FilesScanned} files scanned)]",
                false);
        }

        var fileCount = o.Matches.Select(m => m.Path).Distinct(StringComparer.Ordinal).Count();
        sb.Append($"[search '{v.Pattern}' under {scope.Value}: {o.Matches.Count} match(es) " +
        ...
        {
            sb.Append($"--- {Path.GetRelativePath(scope.Value!, group.Key)} ---\n");
