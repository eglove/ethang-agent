...
public sealed class ExecTool : ITool
{
    ...
    public ToolDefinition Definition { get; } = new(
        ToolName,
        """
        Execute a C# program in the agent workspace. The script runs in-process via Roslyn scripting. The return value is the result: strings verbatim, other objects as one-line JSON. Call Output() during execution for intermediate output. Console.WriteLine is also captured. Thrown exceptions mark the result as an error with exec error [ScriptError] lines. Output over 50,000 characters is truncated with both ends preserved and the full output saved to a file reported as [exec:artifact <path>] — read that file with the read tool. Compile errors report 'exec error [ExecParseError]:' followed by 'line N, col M: message' entries. Timeouts (120s) report 'exec error [ExecTimeout]:' with partial output. Tools are available as methods on the Tools object taking one anonymous object: Tools.read(new { path = "file.txt", startLine = 1, endLine = 50 }). Tools.Invoke("name", args) is the generic form. Tools.List() lists available tools. Shell(exe, args...) runs an external command line through powershell -NoProfile: every argument after the exe is one token of a single native command line (a multi-token piece like "build -c Release" is re-parsed as separate tokens), and the native exit code propagates verbatim. Nested exec is not available. Malformed arguments to exec itself report 'Error [Code]: ...'.
        """,
