...
public sealed class AgentSessionFactory
{
    ...
    /// <summary>Pass a shared <see cref="AppDatabase"/> so every opened session hits
    ///     the ONE app-owned SQLite file (migrated once); when omitted each session
    ///     container constructs its own connection over the default database path.</summary>
    public AgentSessionFactory(AgentSettings settings, string apiKey, ModelConfig defaultModel,
        ...
        IClarifyChannel clarifyChannel, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(workspaceRoot))
            return Result<AgentSession>.Failure(new Error("InvalidWorkspace",
                "workspace root must be a non-empty directory path."));
        if (!Directory.Exists(workspaceRoot))
            return Result<AgentSession>.Failure(new Error("WorkspaceNotFound",
                $"workspace directory not found: '{workspaceRoot}'."));
        if (clarifyChannel is null)
            return Result<AgentSession>.Failure(new Error("InvalidChannel",
        ...
        try
        {
            ...
            var bootstrapped = await RootSessionBootstrapper.PersistRootAsync(store, ct);
            if (!bootstrapped.IsSuccess)
            {
                await services.DisposeAsync();
                return Result<AgentSession>.Failure(bootstrapped.Error!);
            }
