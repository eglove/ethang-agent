...
/// using-superpowers again. Built once per session; no caching needed.</summary>
public sealed class SuperpowersBootstrapPromptProvider : ISystemPromptProvider
{
    public const string SkillName = "using-superpowers";
    ...
    public SuperpowersBootstrapPromptProvider(ISkillCatalog catalog)
    ...
    public string Build()
    {
        ...
        if (!result.IsSuccess || result.Value is null)
            throw new InvalidOperationException(
                ...
                "(packaging defect); cannot build the superpowers bootstrap prompt.");
