...
- **Model Domain**: model capabilities, provider contracts, model configuration, and intelligent model+provider selection (`IModelSelector`, `IModelCatalog`, `IntelligentModelSelector`, `RootAgentResolver`, `ProviderFailoverResolver`, `SessionModelPreferences`). Selection picks a model+provider pair based on cost (after discount), latency, throughput, and capability scores (intelligence, coding, agentic); z.ai's catalog is static and curated (no listing endpoint). Root model selection is deferred to the first user prompt (not startup) and re-runs every 10 user messages; explicit `Model:Id` config pins the model and skips selection. Failover re-selection excludes failed model+provider pairs via a SQLite-backed exclusion store with TTL. `SessionModelPreferences` carries runtime-adjustable knobs (reasoning effort via `/effort`) that the root resolver and child spawner overlay onto resolved configs.
