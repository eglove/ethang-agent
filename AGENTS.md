...
- **Build**: `dotnet build`.
- **Formatting**: `.editorconfig` at the repo root pins conventions — 4-space indent, LF endings, file-scoped namespaces, and the analyzer set enforced by the CI lint job. `dotnet format eThangAgent.slnx` applies or verifies (`--verify-no-changes`) them; run it before committing.
