...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. Per project preference, execution dispatches one Fabric agent per task with review between tasks; repeated agent failure on a task falls back to inline implementation so waves never stall.
