...
**Spec:** `docs/superpowers/specs/2026-08-21-exec-core-design.md` — the plan argues from the spec; executors read both.
...
- `ToolBroker.cs` (+ `ToolInfo`, `ExecToolCallException`) — registry bridge.
...
- Create: `src/eThangAgent.PowerShell.ACL/ToolBroker.cs` (includes `ToolInfo`, `ExecToolCallException`)
