...
> **Progress:** Tasks 1–8 complete and verified green. Next: Task 9 (engine ExecuteAsync), then Waves 5–8. Carry-forward: benign CS8602 at ExecTool.cs:57 (address in Task 14).
...
public class PowerShellExecEngineExecuteTests
{
    ...
    {
        var run = await CreateEngine().ExecuteAsync(new ExecProgram(
            "Invoke-AgentTool -Name read -Input @{ path = 'x.txt'; startLine = 1; endLine = 2 }"));
