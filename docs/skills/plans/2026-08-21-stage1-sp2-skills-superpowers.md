...
- Skill bodies are copied byte-for-byte from `C:\Users\glove\projects\temp-resources\skills-source\skills\<name>\SKILL.md`. NO wording edits to any SKILL.md — ever, including during later tasks.
...
- Upstream source of truth: `C:/Users/glove/projects/temp-resources/skills-source/` (skills under `skills/`).
...
src/eThangAgent.CLI/
  ...
  SkillsBootstrapPromptProvider.cs  # NEW
...
```powershell
$upstream = 'C:/Users/glove/projects/temp-resources/skills-source/skills'
...
description: How methodology-skill action names bind to real eThang Agent tools.
---
...
git commit -m "feat(skill-domain): embed 14 verbatim methodology skills + tool mapping"
```
...
### Task 8: Skills bootstrap prompt provider

**Files:**

- Create: `src/eThangAgent.CLI/SkillsBootstrapPromptProvider.cs`
- Test: `tests/eThangAgent.CLI.Tests/SkillsBootstrapTests.cs`
...
- Produces: `SkillsBootstrapPromptProvider(ISkillCatalog)` whose build output assembles:

```text
<EXTREMELY_IMPORTANT>

{using-skills SKILL.md — byte-for-byte including frontmatter}
...
- [ ] **Step 1: Write failing tests** (`tests/eThangAgent.CLI.Tests/SkillsBootstrapTests.cs`; construct the provider over `new EmbeddedSkillCatalog()` directly):

1. Output starts `<EXTREMELY_IMPORTANT>` and ends `</EXTREMELY_IMPORTANT>`.
2. Contains verbatim frontmatter marker `name: using-skills` and a stable body phrase from the embedded upstream file (open `src/eThangAgent.Skill.Domain/skills/using-skills/SKILL.md`, pick a distinctive sentence, assert `Contains`).
...
6. Provider over a catalog missing using-skills → `InvalidOperationException` (packaging defect = programmer error).
...
```powershell
git add src/eThangAgent.CLI/SkillsBootstrapPromptProvider.cs tests/eThangAgent.CLI.Tests/SkillsBootstrapTests.cs
git commit -m "feat(cli): skills bootstrap prompt provider with inline tool mapping"
```
...
```csharp
[Fact]
public async Task Repl_InjectsSkillsBootstrap_OncePerSession()
{
    ...
    Assert.Contains("name: using-skills", body);
...
git commit -m "feat(cli): inject skills bootstrap at session start with E2E assertion"
```
...
```markdown
- Skill subsystem: 14 embedded development-methodology skills,
